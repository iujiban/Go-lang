...
//구독(채팅방에서 오고가는 메시지를 받는 것) 구조체
type Subscription struct {
	Archive []Event      // 지금까지 쌓인 이벤트를 저장할 슬라이스
	New     <-chan Event // 새 이벤트가 생길 때마다 데이터를 받을 수 있도록 이벤트 채널 생성 (데이터 받기만 하는 것)
}

//이벤트 생성 함수
func NewEvent(evtType, user, msg string) Event {
	return Event{evtType, user, int(time.Now().Unix()), msg} //Event 구조체 현재 시간 맞춘것
}

// 새로운 사용자가 들어왔을 때 이벤트를 구독할 함수
func Subscribe() Subscription {
	c := make(chan Subscription) //채널을 생성하여 구독 채널에 보냄
	subscribe <- c
	return <-c
	...
	for {
		...
		case c := <-subscribe: //새로운 사용자가 들어왔을 때
			...
			for e := archive.Front(); e != nil; e = e.Next() {
				// 쌓인 이벤트가 있다면 events 슬라이스에 이벤트를 저장
...
func main() {
	...
	//웹 브라우저에서 socket.io롤 접속했을 때 실행할 콜백 설정
	server.On("connection", func(so socketio.Socket) {
		...
		//웹 브라우저에서 보내오는 채팅 메세지를 받을 수 있도록 콜백
	...
	//soscket.io 경로는 socket.io 인스턴스가 처리하도록 설정
